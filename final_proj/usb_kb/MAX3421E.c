...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//Create and fill temporary register for writing
	BYTE wr_arr[2] = {reg+2, val};

	//Write data via SPI, get return code
	int ret_code = alt_avalon_spi_command(SPI_0_BASE, 0, 2, &wr_arr, 0, 0, 0);
	if (ret_code < 0) alt_printf("Error. \n");
}
//multiple-byte write
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	//Create and fill temporary register for writing
	BYTE wr_arr[nbytes+1]; //nbytes + 1 as data has size nbytes, reg+2 has size 1
	wr_arr[0] = reg+2;
	...
	//Write data via SPI, get return code
	int ret_code = alt_avalon_spi_command(SPI_0_BASE, 0, nbytes + 1, &wr_arr, 0, 0, 0);
	if (ret_code < 0) alt_printf("Error. \n");
	return (data + nbytes);
}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	// Create variable to hold read value, assuming only single byte being read
	BYTE val;

	// Write reg via SPI, read val via SPI, get return code
	int ret_code = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg, 1, &val, 0);
	if (ret_code < 0) alt_printf("Error. \n");
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	// Write reg via SPI, read data[n] get return code
	int ret_code = alt_avalon_spi_command(SPI_0_BASE, 0, 1, &reg, nbytes, data, 0);
	if (ret_code < 0) alt_printf("Error. \n");
